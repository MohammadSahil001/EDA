...
1.  **Datasets:** Two datasets were used:
...
2.  **Merging:** The two datasets were merged into a single DataFrame (`final_df`) using the `Country Code` as the common key.
...
The `Aggregate rating`, `Rating color`, and `Rating text` columns were analyzed to understand the rating system.
  ...
  * **Rating vs. Count:** A bar plot of `Aggregate rating` vs. `Rating Count` shows two key findings:
    ...
    1.  A very large number of restaurants (**2148**) are **"Not rated"** (0.0 rating).
    2.  The majority of rated restaurants fall in the **"Average"** range (2.5 to 3.4).
